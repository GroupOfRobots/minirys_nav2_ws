...
void MultipleRobotsLayer::onInitialize()
{
  ...
  this->robotsNamespacesSubscribtion = node->create_subscription<minirys_msgs::msg::RobotsNamespaces>(
            "robots_namespaces", 10, std::bind(&MultipleRobotsLayer::receiveRobotsNamespaces, this, std::placeholders::_1));
}
...
void MultipleRobotsLayer::receiveRobotsNamespaces(const minirys_msgs::msg::RobotsNamespaces::SharedPtr msg) {
  ...
  {
      RCLCPP_INFO(rclcpp::get_logger("nav2_costmap_2d"), "Namespaces have changed.");

      // Update the namespaces vector
      this->robots_namespaces_ = new_namespaces;

      // Print the updated namespaces for debugging purposes
      // RCLCPP_INFO(rclcpp::get_logger("nav2_costmap_2d"), "Updated namespaces:");
      // for (const auto& ns : this->robots_namespaces_) {
      //     RCLCPP_INFO(rclcpp::get_logger("nav2_costmap_2d"), "%s", ns.c_str());
      // }
  }
}
...
void MultipleRobotsLayer::updateBounds(
...
void MultipleRobotsLayer::onFootprintChanged()
{
  ...
  RCLCPP_DEBUG(rclcpp::get_logger(
      "nav2_costmap_2d"), "MultipleRobotsLayer::onFootprintChanged(): num footprint points: %lu",
...
void MultipleRobotsLayer::updateCosts(
